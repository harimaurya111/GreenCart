...
const Cart = () => {
    ...
    const { products, cartItems, currency, updateCartItem, removeFromCart, getCartCount, getCartAmount, navigate, axios, toast, user, setCartItems } = useAppContext()
    ...
    const placeOrder = async () => {
        try {
            if (Object.keys(cartItems).length === 0) {
                toast.error("Pls add product in cart")
                navigate('/products')
                return
            }
        ...
        <div className="flex flex-col md:flex-row mt-16">
            <div className='flex-1 max-w-4xl'>
                <h1 className="text-3xl font-medium mb-6">
                    Shopping Cart : <span className="text-2xl font-semibold text-primary">{getCartCount()!==0 && getCartCount()}</span>
                </h1>
                ...
                {cartArray && cartArray.length > 0 ? cartArray.map((product, index) =>
...
(
                     ...
                     <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                         ...
                         <button onClick={() => removeFromCart(product._id)} className="cursor-pointer mx-auto">
                             <img src={assets.remove_icon} alt="remove" className="inline-block w-6h-4" />
                         </button>
                     </div>)
